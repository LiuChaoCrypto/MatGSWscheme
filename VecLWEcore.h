...
class VecLWEparams_uint16 {
public:
	VecLWEparams_uint16() :m_N(0), m_Q(0), m_sigma(0), m_q(0), m_l(0),m_t(0),m_delta(0) {}

	explicit VecLWEparams_uint16(uint32_t t_N, uint32_t t_Q, float t_sigma, uint32_t t_q,uint32_t t_t)
...
public:
	...
	explicit VecLWECiphertext_uint16(const vector<uint16_t>& a, const vector<uint16_t>& b) : vec_a(a), vec_b(b) {}
...
public:
	VecLWEPlaintext_uint16() {}

	explicit VecLWEPlaintext_uint16(const vector<uint16_t>& m) : vec_m(m) {}
