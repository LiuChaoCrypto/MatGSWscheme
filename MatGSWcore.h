...
class MatGSWCiphertext_uint16 {
public:
	MatGSWCiphertext_uint16() {}
	explicit MatGSWCiphertext_uint16(const vector<vector<uint16_t>>& a, const vector<vector<uint16_t>>& b) : mat_A(a), mat_B( b) {}

	const vector<vector<uint16_t>>& GetA() const { return mat_A; }
...
private:
	vector<vector<uint16_t>> mat_A;
	vector<vector<uint16_t>> mat_B;

};

class MatGSWSecretKey_uint16 {
public:
...
public:
	MatGSWPlaintext_uint16() {}

	explicit MatGSWPlaintext_uint16(const vector<vector<uint16_t>>& m) : mat_M(m) {}
